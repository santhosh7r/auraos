...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-card shadow-premium transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-300 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, hideClose, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      {!hideClose && (
        <SheetPrimitive.Close className="absolute right-4 top-4 rounded-lg p-1 opacity-60 transition-opacity hover:opacity-100 hover:bg-accent focus:outline-none">
          <X className="size-4" />
